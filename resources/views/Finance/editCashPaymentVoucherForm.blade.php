			...
			<div class="row">
				<?php 
					if($pvsDetail->pv_status == 1){
						echo Form::open(array('url' => 'fad/editCashPaymentPendingVoucherDetail?m='.$m.'','id'=>'cashPaymentVoucherForm'));
				}else if($pvsDetail->pv_status == 2){
					echo Form::open(array('url' => 'fad/editCashPaymentApproveVoucherDetail?m='.$m.'','id'=>'cashPaymentVoucherForm'));
				}
				?>
...
<script>
	...
	function formSubmitOne(e){
		...
		var postData = $('#cashPaymentVoucherForm').serializeArray();
		var formURL = $('#cashPaymentVoucherForm').attr("action");
		$.ajax({
        	url : formURL,
            type: "POST",
            data : postData,
